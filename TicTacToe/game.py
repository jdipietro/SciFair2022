...
class TicTacToeGame():
    def __init__(self):
        ...
        self.sides = [1, 3, 5, 7]
        self.middle = 4
        self.player_marker = 0
        self.current_player_marker_index = 0

    def get_marker(self):
        return (1.0, 0.0)

    def reset(self):
        self.player_marker = random.randint(0,1)
        self.board = [-1.0] * 9
        state = list(self.board)
        state.append (self.player_marker)
        return state
    ...
    def step(self, action):
        ...
        if not self.is_space_free(self.board, action):
            self.player_marker = (self.player_marker + 1) % 2
            state = list(self.board)
            state.append(self.player_marker)
